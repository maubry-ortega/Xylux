...
pub const fn register_winsize_change_signal_handler() -> Result<(), Error> {
    Ok(())
}
...
#[allow(clippy::unnecessary_wraps)] // Result required on other platforms
pub const fn set_term_mode(_term: &TermMode) -> Result<(), Error> {
    Ok(())
}

// Opening the file /dev/tty is effectively the same as `raw_mode`
#[allow(clippy::unnecessary_wraps)] // Result required on other platforms
pub const fn enable_raw_mode() -> Result<TermMode, Error> {
    Ok(TermMode {})
}
